...
func String2IpLittle(ip string) int32 {
	bytes := strings.Split(ip, ".")
	val := 0
	for k, v := range bytes {
		i, err := strconv.Atoi(v)
...
func Ip2StringLittle(ip int32) string {
	return fmt.Sprintf("%d.%d.%d.%d", ip&0xff, (ip>>8)&0xff, (ip>>16)&0xff, (ip>>24)&0xff)
}

func HostInt16ToLittle(i int16) int16 {
	vec := []byte{byte(i >> 8), byte(i)}
	return int16(binary.LittleEndian.Uint16(vec))
}
